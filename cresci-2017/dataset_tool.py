...
def fast_merge(dataset="cresci-2015", server_id="209"):
    assert dataset in dataset_names, f"Invalid dataset {dataset}"
    node_info = pd.read_json("../datasets/cresci-2015/node.json")
    ...
    for i in tqdm(range(length)):
        if user["id"].iloc[i] in id_to_label.keys():
            if i < len(label):
                user.loc[i, "label"] = id_to_label[user["id"].iloc[i]]
                user.loc[i, "split"] = id_to_split[user["id"].iloc[i]]
            else:
