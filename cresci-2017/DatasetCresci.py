...
class Twibot22(Dataset):
    def __init__(self, root='./processed_data/', device='cuda:0'):
        self.root = root
        self.device = device
        self.save = True
    ...
    # cresci-15 dataset does not have tweet,return the all zero tensor first
    def tweets_embedding(self):
        return torch.zeros((5301, 768)).to(self.device)
    ...
    def tweets_preprocess(self):
        ...
        tweets=np.load(self.root+'each_user_tweets.npy',allow_pickle=True)
        print('Finished')
        return tweets
    ...
    def train_val_test_mask(self):
        if self.root == './processed_data/':
            train_idx = range(3801)
            val_idx = range(3801, 3801+1000)
            test_idx = range(3801+1000, 3801+1000+499)
        else:
