...
class Twibot20(Dataset):
    def __init__(self,root='./processed_data/',device='cpu',process=True,save=True):
        ...
        if process:
            ...
            df_train=pd.read_json('../datasets/Twibot-20/train.json')
            print('Loading test.json')
            df_test=pd.read_json('../datasets/Twibot-20/test.json')
            print('Loading support.json')
            df_support=pd.read_json('../datasets/Twibot-20/support.json')
            print('Loading dev.json')
            df_dev=pd.read_json('../datasets/Twibot-20/dev.json')
            print('Finished')
    ...
    def Des_embbeding(self):
        ...
        if not os.path.exists(path):
            ...
            feature_extraction = pipeline('feature-extraction', model='../roberta-base', tokenizer='../roberta-base',
                                       device=0, padding=True, truncation=True, max_length=50, add_special_tokens=True)
            des_vec=[]
    ...
    def tweets_embedding(self):
        ...
        if not os.path.exists(path):
            tweets = np.load(self.root+"tweets.npy", allow_pickle=True)
            print('Loading RoBerta')
    ...
    def num_prop_preprocess(self):
        ...
        if not os.path.exists(path0):
            ...
            if self.save:
                torch.save(num_prop, self.root + "num_properties_tensor.pt")
