...
class oriRGCN(nn.Module):
    def __init__(self, des_size=768, tweet_size=768, num_prop_size=6, cat_prop_size=1, embedding_dimension=128,
                 dropout=0.3):
        super(oriRGCN, self).__init__()
        self.dropout = dropout
        self.linear_relu_des = nn.Sequential(
            # nn.Linear(输入向量大小768,输出向量大小8)
...
class RGCNDetector(nn.Module):
    def __init__(self, numeric_num=5, cat_num=1, linear_channels=128, tweet_channel=768, des_channel=768, out_channel=128, dropout=0.5):
        super(RGCNDetector, self).__init__()
        ...
        self.RGCN_layer1 = RGCNConv(in_channels=linear_channels, out_channels=linear_channels, num_relations=2, bias=False)
        self.RGCN_layer2 = RGCNConv(in_channels=linear_channels, out_channels=linear_channels, num_relations=2, bias=False)
    ...
    def forward(self, des_features, tweet_features, prop_features, cat_features, edge_index, edge_type):
        ...
        user_features = self.RGCN_layer1(user_features, edge_index, edge_type)
        user_features = self.RGCN_layer1(user_features, edge_index, edge_type)
...
class HGTDetector(nn.Module):
    def __init__(self, num_prop_size=6, cat_prop_size=11, linear_channels=128, tweet_channel=768, des_channel=768, out_channel=128,
                 dropout=0.5):
        super(HGTDetector, self).__init__()

        self.in_linear_numeric = nn.Linear(num_prop_size, int(linear_channels / 4), bias=True)
        self.in_linear_bool = nn.Linear(cat_prop_size, int(linear_channels / 4), bias=True)
        self.in_linear_tweet = nn.Linear(tweet_channel, int(linear_channels / 4), bias=True)
        self.in_linear_des = nn.Linear(des_channel, int(linear_channels / 4), bias=True)
        self.linear1 = nn.Linear(linear_channels, linear_channels)
