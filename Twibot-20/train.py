...
device = 'cuda:0'
embedding_size, dropout, lr, weight_decay = 128, 0.1, 1e-3, 5e-2
root = './processed_data/'
...
model = oriRGCN(num_prop_size=6, cat_prop_size=11, embedding_dimension=embedding_size).to(device)
# model = BotRGCN(num_prop_size=6, cat_prop_size=11, embedding_dimension=embedding_size).to(device)
# model = HGTDetector(num_prop_size=6, cat_prop_size=11, linear_channels=embedding_size).to(device)
...
# torch.save(model.state_dict('model/model_Bot_RGCN.pth'))
